...
var qs = require("querystring");  //to normalize JSON into RESTFul parameters
...
function KeenApi(config) {
    ...
    this.queries = {
        perform: function(collection, metric, query, callback) {
            if (!this.readKey) {
            ...
            if (!collection) {
                var errorMessage = "You must specify a non-null, non-empty collection name!";
                var error = new Error(errorMessage);
                if (callback) {
                    callback(error);
                } else {
            ...
            if (!metric) {
                var errorMessage = "You must specify a non-null, non-empty metric name!";
                var error = new Error(errorMessage);
                if (callback) {
                    callback(error);
                } else {
        ...
        __proto__: this //necessary to access parent config data
    }
}
